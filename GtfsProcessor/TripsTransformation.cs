            ...
            for (int i = 0; i < ownerTrip.StopTimes.Length; i++)
            {
                ...
                if (stopTime.IsPublic)
                {
                    ...
                    var gtfsStopTime = new GtfsModel.Extended.StopTime()
                    {
                        ...
                        DropOffType = stopTime.IsRequestStop ? DropOffType.DriverRequest : DropOffType.Regular,
                        PickupType = stopTime.IsRequestStop ? PickupType.DriverRequest : PickupType.Regular,
