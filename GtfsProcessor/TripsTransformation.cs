            ...
            for (int i = 0; i < ownerTrip.StopTimes.Length; i++)
            {
                ...
                if (stopTime.IsPublic)
                {
                    ...
                    var gtfsStopTime = new GtfsModel.Extended.StopTime()
                    {
                        ...
                        DropOffType = stopTime.BoardingOnly ? DropOffType.None : stopTime.IsOnPhoneRequest ? DropOffType.PhoneRequest : stopTime.IsRequestStop ? DropOffType.DriverRequest : DropOffType.Regular,
                        PickupType = stopTime.ExitOnly ? PickupType.None : stopTime.IsOnPhoneRequest ? PickupType.PhoneRequest : stopTime.IsRequestStop ? PickupType.DriverRequest : PickupType.Regular,
